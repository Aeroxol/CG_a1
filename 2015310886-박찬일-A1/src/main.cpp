...
uint				NUM_CIRCLE = 20;		// number of circles
...
void update_vertex_buffer( const std::vector<vertex>& vertices, uint N )
{
	...
	// create buffers
	if(b_index_buffer)	//index buffering
	{
		...
		// ELEMENT_ARRAY_BUFFER == INDEX_BUFFER
...
void keyboard( GLFWwindow* window, int key, int scancode, int action, int mods )
{
	if(action==GLFW_PRESS)
	{
		if(key==GLFW_KEY_ESCAPE||key==GLFW_KEY_Q)	glfwSetWindowShouldClose( window, GL_TRUE );
			...
			if (NUM_CIRCLE < 30) {
				NUM_CIRCLE++;
				printf("> number of circles increased to %d\n", NUM_CIRCLE);
				circles.clear();
			...
			if (NUM_CIRCLE > 20) {
				NUM_CIRCLE--;
				printf("> number of circles decreased to %d\n", NUM_CIRCLE);
				circles.clear();
				circles = std::move(create_circles(NUM_CIRCLE));
			}
		}
